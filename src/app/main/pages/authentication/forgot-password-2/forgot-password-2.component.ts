...
export class ForgotPassword2Component implements OnInit
{
   ...
   async forgetpassword(){
        await this.AuthService.forgetPassword(this.forgotPasswordForm.value).subscribe(response => {
            this.Router.navigate(['/pages/auth/mail-confirm', this.forgotPasswordForm.get('email')?.value]);
        }),error => {  
